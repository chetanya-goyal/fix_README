...
from glayout.flow.pdk.util.comp_utils import evaluate_bbox, prec_array, to_float, move, prec_ref_center, to_decimal, component_array
from glayout.flow.pdk.util.port_utils import rename_ports_by_orientation, print_ports, get_unique_ports
from glayout.flow.pdk.util.snap_to_grid import component_snap_to_grid
from decimal import Decimal
from typing import Literal
import gdsfactory as gf
from glayout.flow.routing.hashport import HPort, init_hashport
from gdsfactory.pdk import get_active_pdk, get_layer
...
def __get_viastack_minseperation(pdk: MappedPDK, viastack: Component, ordered_layer_info) -> tuple[float,float]:
    ...
    get_sep = lambda _pdk, rule, _lay_, comp : (rule+2*comp.extract(layers=[_pdk.get_glayer(_lay_)]).dxmax)
    ...
    via_spacing = snap_to_grid(max(via_spacing), nm = 10)
    top_enclosure = snap_to_grid(2 * top_enclosure, nm = 10)
    return [via_spacing,top_enclosure]
    ...
    if level1 == level2:
        ...
        for level in range(level1,level2+1):
            ...
            if level != level2:
                ...
                via_dim = snap_to_grid(via_dim, nm = 10)
                via_ref = viastack << rectangle(size=(via_dim,via_dim),layer=pdk.get_glayer(via_name), centered=True)
            layer_dim = snap_to_grid(1 * layer_dim, nm = 10)
            # layer_dim = snap_to_grid(1.4 * layer_dim, nm = 10)
        ...
        if fullbottom:
            dim = evaluate_bbox(viastack)
            width = snap_to_grid(dim[0], nm = 10)
            length = snap_to_grid(dim[1], nm = 10)
            bot_ref = viastack << rectangle(size=(width, length),layer=pdk.get_glayer(glayer1), centered=True)
            if level1!=0:
        ...
        if not centered:
            viastack = move(viastack,(viastack.dxmax,viastack.dymax))
    return component_snap_to_grid(rename_ports_by_orientation(viastack), flatten_true=True)
...
def via_array(
    ...
    ****NOTE: this implies lay_bottom
    ...
    viadim = snap_to_grid(viadim, nm = 10)
    ...
    # create array
    # print('new import pdb')
    # viarray_ref = c.add_ref(viastack, columns=cnum_vias[0], rows = cnum_vias[1], spacing = 2 * [via_abs_spacing])
    rows = cnum_vias[1]
    columns = cnum_vias[0]
    spacing = (via_abs_spacing, via_abs_spacing)
   
    viaarray_ref = viaarray << component_array(component = viastack, spacing = spacing, rows = rows, columns = columns, centered = True)
    ...
    viadims = snap_to_grid(viadims, nm = 10)
    if not size:
        size = (None, None)
    size = tuple(snap_to_grid(size[i], nm = 10) if size[i] else viadims[i] for i in range(2))
    size = tuple(snap_to_grid(viadims[i], nm = 10) if viadims[i]>size[i] else size[i] for i in range(2))
    ...
    if lay_bottom or fullbottom or lay_every_layer:
        ...
        newsize = size if fullbottom else bdims
        bref = viaarray << rectangle(size=tuple(snap_to_grid(newsize[i], nm = 10) for i in range(2)), layer=pdk.get_glayer(glayer1), centered=True)
        viaarray.add_ports(bref.ports, prefix="bottom_lay_")
    else:
        viaarray = viaarray.remove_layers(layers=[pdk.get_glayer(glayer1)])
    # place top met
    # tref_size = evaluate_bbox(viaarray.extract(layers=[pdk.get_glayer(glayer2)]))
    # tref = viaarray << rectangle(size=tref_size, layer=pdk.get_glayer(glayer2), centered=True)
    ...
    viaarray = rename_ports_by_orientation(viaarray)
    # viaarray.flatten()
    return viaarray
